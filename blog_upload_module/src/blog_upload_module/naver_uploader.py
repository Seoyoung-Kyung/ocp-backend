...
class NaverBlogAutomation:
    """
    네이버 블로그 자동 업로드 봇 (Docker 환경 대응)
    - 제목: DOM + ActionChains
    - 본문: JavaScript 직접 주입 (클립보드 우회)
    """
    ...
    # ------------------------------------------------------------------
    # HTML 콘텐츠 준비 (클립보드 사용하지 않음 - Docker 환경 대응)
    # ------------------------------------------------------------------
    def _copy_html_as_rich(self, html: str):
        """
        HTML 콘텐츠를 인스턴스 변수에 저장
        클립보드 대신 JavaScript 직접 주입 사용
        """
        logger.info("HTML 콘텐츠 준비 (클립보드 우회)")
        self._prepared_html = html
        logger.info("HTML 콘텐츠 준비 완료")
    ...
    def _paste_body_in_naver_blog(self):
        """
        본문을 네이버 블로그에 직접 주입 (클립보드 우회 - Docker 환경 대응)
        JavaScript를 사용하여 contenteditable 영역에 HTML을 직접 삽입
        """
        driver = self.driver
        ...
        logger.info("네이버 본문 영역에 HTML 직접 주입 시도")

        # _prepared_html이 있는지 확인
        if not hasattr(self, '_prepared_html') or not self._prepared_html:
            logger.error("준비된 HTML 콘텐츠가 없습니다")
            return

        try:
            # JavaScript로 contenteditable 영역에 HTML 직접 삽입
            success = driver.execute_script(
                ...
                if (!target) {
                    console.error('contenteditable 영역을 찾을 수 없습니다');
                    return false;
                }
            ...
            if success:
                logger.info("네이버 본문 HTML 직접 주입 완료")
                time.sleep(1.8)
            else:
                logger.error("JavaScript HTML 주입 실패 - target 영역을 찾을 수 없음")
                raise RuntimeError("본문 영역을 찾을 수 없습니다")

        except Exception as exc:
            logger.error(f"네이버 본문 주입 실패: {exc}")
            raise
