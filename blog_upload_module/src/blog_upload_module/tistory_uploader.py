...
class TistoryBlogAutomation:
    ...
    def paste_content_like_human(self, content: str, timeout: int = 15):
        """
        CodeMirror 에디터에 HTML 콘텐츠를 직접 주입
        클립보드를 사용하지 않고 JavaScript로 직접 설정 (Docker 환경 대응)
        """
        driver = self.driver
        ...
        logger.info("CodeMirror 에디터 발견 - JavaScript 직접 주입 시도")

        try:
            # JavaScript로 CodeMirror 값 직접 설정
            ...
            logger.info("CodeMirror 콘텐츠 직접 주입 완료")
            time.sleep(max(1.0, min(3, len(content) / 5000)))  # 최소 1초 대기
            return True

        except Exception as e:
            logger.error(f"CodeMirror 직접 주입 실패: {e}")
            return False
